...
static void
generate_h_file( FILE* fp, const char* module_name, bool include_default_types, const parsed_data_t* data )
{
    UNUSED(data);
    UNUSED(include_default_types);
    ...
    // All modules, even if they don't own the default types, may need to reference them.
    // So we provide extern declarations for all modules.
    // The main executable's generated C file will contain the actual definitions.
    // clang-format off
                 ...
                 const file_list_t*   headers )
{
    ...
    if ( ( include_default_types ) || ( data->num_types > 0 ) )
    {
    ...
    file_print_fmt( fp, "void %s_register_types(void) {\n", module_name );
    file_print_fmt( fp, "    cf_register_table(%s_type_array, %s_type_count);\n", module_name, module_name );
    file_print_fmt( fp, "}\n" );
}
                       ...
                       const file_list_t*   headers )
{
    ...
    generate_h_file( fp_h, module_name, include_default_types, data );
    fclose( fp_h );
    ...
    generate_c_file( fp_c, module_name, include_default_types, data, headers );
    fclose( fp_c );
    print_fmt( "Generated %s\n", c_path );
